...
<html>
<head>
<title>TES | Tourism History</title>
...
<div class="banner-1 ">
	<div class="container">
		<h1 class="wow zoomIn animated animated" data-wow-delay=".5s" style="visibility: visible; animation-delay: 0.5s; animation-name: zoomIn;">TES - Tourism History</h1>
	</div>
</div>
<!--- /banner-1 ---->
<!--- privacy ---->
...
<td>
	...
	$sql2="SELECT * FROM tblbooking WHERE BookingId=$bookid";
	$res1=mysqli_query($db,$sql2);
	$data=mysqli_fetch_assoc($res1);
	$pay=$data['pay'];
		if($result->status==1){
			if($pay==null){
				$_SESSION['bookid']=htmlentities($result->bookid);
				echo $pay;
                  ?>
                  <div class="vehicle_status"><a href="payment.php" class="btn outline btn-xs active-btn">Pay </a></div>
                <?php

			}else{
			?>
			<div class="vehicle_status"><a href="#" class="btn outline btn-xs active-btn">Paid</a></div>
		  <?php 
